...
class SD15(supported_models_base.BASE):
    ...
    def process_clip_state_dict(self, state_dict):
        ...
        state_dict = comfy_utils.state_dict_prefix_replace(state_dict, replace_prefix, filter_keys=True)
        return state_dict
    ...
    def process_clip_state_dict_for_saving(self, state_dict):
        ...
        return comfy_utils.state_dict_prefix_replace(state_dict, replace_prefix)
...
class SD20(supported_models_base.BASE):
    ...
    def process_clip_state_dict(self, state_dict):
        ...
        state_dict = comfy_utils.state_dict_prefix_replace(state_dict, replace_prefix, filter_keys=True)
        state_dict = comfy_utils.clip_text_transformers_convert(state_dict, "clip_h.", "clip_h.transformer.")
        return state_dict

    def process_clip_state_dict_for_saving(self, state_dict):
        replace_prefix = {}
        replace_prefix["clip_h"] = "cond_stage_model.model"
        state_dict = comfy_utils.state_dict_prefix_replace(state_dict, replace_prefix)
        state_dict = diffusers_convert.convert_text_enc_state_dict_v20(state_dict)
        return state_dict

    def clip_target(self, state_dict={}):
        return supported_models_base.ClipTarget(comfy_api.text_encoders.sd2_clip.SD2Tokenizer, comfy_api.text_encoders.sd2_clip.SD2ClipModel)
...
class SDXLRefiner(supported_models_base.BASE):
    ...
    def process_clip_state_dict(self, state_dict):
        ...
        state_dict = comfy_utils.clip_text_transformers_convert(state_dict, "clip_g.", "clip_g.transformer.")
        state_dict = comfy_utils.state_dict_key_replace(state_dict, keys_to_replace)
        return state_dict
    ...
    def process_clip_state_dict_for_saving(self, state_dict):
        ...
        state_dict_g = comfy_utils.state_dict_prefix_replace(state_dict_g, replace_prefix)
        return state_dict_g
...
class Stable_Cascade_C(supported_models_base.BASE):
    ...
    def process_clip_state_dict(self, state_dict):
        state_dict = comfy_utils.state_dict_prefix_replace(state_dict, {k: "" for k in self.text_encoder_key_prefix}, filter_keys=True)
...
class SD3(supported_models_base.BASE):
    ...
    def clip_target(self, state_dict={}):
        ...
        if "{}clip_g.transformer.text_model.final_layer_norm.weight".format(pref) in state_dict:
            clip_g = True
        t5_detect = comfy_api.text_encoders.sd3_clip.t5_xxl_detect(state_dict, "{}t5xxl.transformer.".format(pref))
        if "dtype_t5" in t5_detect:
            t5 = True

        return supported_models_base.ClipTarget(comfy_api.text_encoders.sd3_clip.SD3Tokenizer, comfy_api.text_encoders.sd3_clip.sd3_clip(clip_l=clip_l, clip_g=clip_g, t5=t5, **t5_detect))
...
class StableAudio(supported_models_base.BASE):
    ...
    def get_model(self, state_dict, prefix="", device=None):
        seconds_start_sd = comfy_utils.state_dict_prefix_replace(state_dict, {"conditioner.conditioners.seconds_start.": ""}, filter_keys=True)
        seconds_total_sd = comfy_utils.state_dict_prefix_replace(state_dict, {"conditioner.conditioners.seconds_total.": ""}, filter_keys=True)
    ...
    def clip_target(self, state_dict={}):
        return supported_models_base.ClipTarget(comfy_api.text_encoders.sa_t5.SAT5Tokenizer, comfy_api.text_encoders.sa_t5.SAT5Model)
...
class AuraFlow(supported_models_base.BASE):
    ...
    def clip_target(self, state_dict={}):
        return supported_models_base.ClipTarget(comfy_api.text_encoders.aura_t5.AuraT5Tokenizer, comfy_api.text_encoders.aura_t5.AuraT5Model)
...
class PixArtAlpha(supported_models_base.BASE):
    ...
    def clip_target(self, state_dict={}):
        return supported_models_base.ClipTarget(comfy_api.text_encoders.pixart_t5.PixArtTokenizer, comfy_api.text_encoders.pixart_t5.PixArtT5XXL)
...
class HunyuanDiT(supported_models_base.BASE):
    ...
    def clip_target(self, state_dict={}):
        return supported_models_base.ClipTarget(comfy_api.text_encoders.hydit.HyditTokenizer, comfy_api.text_encoders.hydit.HyditModel)
...
class Flux(supported_models_base.BASE):
    ...
    def clip_target(self, state_dict={}):
        pref = self.text_encoder_key_prefix[0]
        t5_detect = comfy_api.text_encoders.sd3_clip.t5_xxl_detect(state_dict, "{}t5xxl.transformer.".format(pref))
        return supported_models_base.ClipTarget(comfy_api.text_encoders.flux.FluxTokenizer, comfy_api.text_encoders.flux.flux_clip(**t5_detect))
...
class GenmoMochi(supported_models_base.BASE):
    ...
    def clip_target(self, state_dict={}):
        pref = self.text_encoder_key_prefix[0]
        t5_detect = comfy_api.text_encoders.sd3_clip.t5_xxl_detect(state_dict, "{}t5xxl.transformer.".format(pref))
        return supported_models_base.ClipTarget(comfy_api.text_encoders.genmo.MochiT5Tokenizer, comfy_api.text_encoders.genmo.mochi_te(**t5_detect))
...
class LTXV(supported_models_base.BASE):
    ...
    def clip_target(self, state_dict={}):
        pref = self.text_encoder_key_prefix[0]
        t5_detect = comfy_api.text_encoders.sd3_clip.t5_xxl_detect(state_dict, "{}t5xxl.transformer.".format(pref))
        return supported_models_base.ClipTarget(comfy_api.text_encoders.lt.LTXVT5Tokenizer, comfy_api.text_encoders.lt.ltxv_te(**t5_detect))
...
class HunyuanVideo(supported_models_base.BASE):
    ...
    def clip_target(self, state_dict={}):
        pref = self.text_encoder_key_prefix[0]
        hunyuan_detect = comfy_api.text_encoders.hunyuan_video.llama_detect(state_dict, "{}llama.transformer.".format(pref))
        return supported_models_base.ClipTarget(comfy_api.text_encoders.hunyuan_video.HunyuanVideoTokenizer, comfy_api.text_encoders.hunyuan_video.hunyuan_video_clip(**hunyuan_detect))
