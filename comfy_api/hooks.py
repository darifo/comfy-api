...
if TYPE_CHECKING:
    from comfy_api.model_patcher import ModelPatcher, PatcherInjection
    from comfy_api.model_base import BaseModel
    from comfy_api.sd import CLIP
import comfy_api.lora
import comfy_api.model_management
import comfy_api.patcher_extension
from nodes.node_helpers import conditioning_set_values
    ...
    def add_hook_patches(self, model: 'ModelPatcher', model_options: dict, target: EnumWeightTarget, registered: list[Hook]):
        ...
        if self.need_weight_init:
            ...
            if target == EnumWeightTarget.Model:
                ...
                key_map = comfy_api.lora.model_lora_keys_clip(model.model, key_map)
            weights = comfy_api.lora.load_lora(self.weights, key_map, log_missing=False)
        else:
            if target == EnumWeightTarget.Model:
...
class WrapperHook(Hook):
    ...
    def add_hook_patches(self, model: 'ModelPatcher', model_options: dict, target: EnumWeightTarget, registered: list[Hook]):
        ...
        comfy_api.patcher_extension.merge_nested_dicts(model_options, add_model_options, copy_dict1=False)
        registered.append(self)
        return True
...
def load_hook_lora_for_models(model: 'ModelPatcher', clip: 'CLIP', lora: dict[str, torch.Tensor],
    ...
    hook = WeightHook()
    hook_group.add(hook)
    loaded: dict[str] = comfy_api.lora.load_lora(lora, key_map)
    if model is not None:
