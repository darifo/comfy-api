...
class Nodes:
    ...
    # 图片缩放按系数
    def image_upscale_by_coefficient(self, image, upscale_method, scale_by):
        samples = image.movedim(-1,1)
        width = round(samples.shape[3] * scale_by)
        height = round(samples.shape[2] * scale_by)
        s = comfy_api.comfy_utils.common_upscale(samples, width, height, upscale_method, "disabled")
        s = s.movedim(1,-1)
        return (s,)
    ...
    # 图像按照遮罩裁剪
    def image_crop_by_mask(self, masks, original_images, crop_size_mult, bbox_smooth_alpha):
        ...
        self.max_bbox_width = node_helpers.smooth_bbox_size(self.max_bbox_width, curr_max_bbox_width, bbox_smooth_alpha)
        self.max_bbox_height = node_helpers.smooth_bbox_size(self.max_bbox_height, curr_max_bbox_height, bbox_smooth_alpha)
        ...
        for i, (mask, img) in enumerate(zip(masks, original_images)):
            ...
            if i > 0:
                center = node_helpers.smooth_center(self.prev_center, curr_center, bbox_smooth_alpha)
            else:
                center = curr_center
